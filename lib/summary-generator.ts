// Summary generation utilities for comment analysis
...
export function generateSummary(
  ...
  // Identify top concerns from negative comments
...
function generateExecutiveSummary(summary: any, totalComments: number): string {
  ...
  const sentimentStrength = Math.max(summary.positivePercentage, summary.negativePercentage, summary.neutralPercentage)
  ...
  return `Analysis of ${totalComments} public comments reveals a ${tone} ${dominantSentiment} overall sentiment (${sentimentStrength}%). The average sentiment score of ${summary.averageScore.toFixed(2)} indicates ${
...
function generateKeyFindings(summary: any, keywords: Array<{ word: string; count: number }>): string[] {
  const findings = []
  ...
  // Dominant sentiment finding
  if (summary.positivePercentage > 50) {
    findings.push("Majority of respondents express favorable views toward the proposed policy")
  } else if (summary.negativePercentage > 50) {
    findings.push("Majority of respondents express concerns or opposition to the proposed policy")
  } else {
  ...
  if (summary.total > 100) {
    findings.push("High level of public engagement with significant participation in the consultation process")
  } else if (summary.total > 50) {
    findings.push("Moderate level of public engagement in the consultation process")
  } else {
...
function generateSentimentOverview(summary: any): string {
  const { positivePercentage, negativePercentage, neutralPercentage, averageScore } = summary
  ...
  if (Math.abs(averageScore) > 0.3) {
    overview += `The strong average sentiment score (${averageScore.toFixed(2)}) indicates clear public opinion trends.`
  } else {
    overview += `The moderate average sentiment score (${averageScore.toFixed(2)}) suggests nuanced public opinion.`
  }

  return overview
}

function generateTopConcerns(results: CommentAnalysis[], keywords: Array<{ word: string; count: number }>): string[] {
  const concerns = []

  // Get negative comments
  const negativeComments = results.filter((r) => r.analysis.sentiment.label === "negative")
  ...
  const identifiedConcerns = keywords.filter((k) => concernKeywords.includes(k.word.toLowerCase()))

  if (identifiedConcerns.length > 0) {
    concerns.push(
      `Financial concerns mentioned ${identifiedConcerns.find((k) => ["cost", "expensive", "burden"].includes(k.word))?.count || 0} times`,
    )
  }

  // Generic concerns based on negative sentiment
  const negativePercentage = (negativeComments.length / results.length) * 100
  if (negativePercentage > 30) {
    concerns.push("Significant portion of respondents express reservations about the proposal")
  }

  if (negativePercentage > 50) {
    concerns.push("Majority opposition suggests need for policy revision or additional consultation")
  }

  // Add top keywords from negative comments as potential concerns
  ...
  if (topNegativeKeywords.length > 0) {
    concerns.push(`Key areas of concern include: ${topNegativeKeywords.join(", ")}`)
  }

  return concerns.length > 0 ? concerns : ["No specific concerns identified in the analysis"]
}
...
function generateRecommendations(summary: any, concerns: string[]): string[] {
  ...
  if (summary.negativePercentage > 40) {
    recommendations.push("Consider addressing the primary concerns raised by respondents before policy implementation")
    recommendations.push("Conduct additional stakeholder engagement to better understand opposition viewpoints")
  }

  if (summary.positivePercentage > 60) {
    recommendations.push("Leverage positive public sentiment to build support for policy implementation")
    recommendations.push("Highlight the aspects of the policy that resonate most positively with the public")
  }

  if (summary.neutralPercentage > 40) {
    recommendations.push("Provide additional information to help neutral respondents form more definitive opinions")
    recommendations.push("Focus on education and awareness campaigns to clarify policy benefits and impacts")
  }

  // Based on concerns
  if (concerns.some((c) => c.includes("cost") || c.includes("Financial"))) {
    recommendations.push("Develop clear cost-benefit analysis and communicate economic impacts transparently")
  }

  if (summary.total < 50) {
    recommendations.push("Expand outreach efforts to increase public participation in future consultations")
  }

  // General recommendations
  recommendations.push("Monitor ongoing public sentiment as policy development progresses")
  recommendations.push("Consider implementing a feedback mechanism for continuous public input")
